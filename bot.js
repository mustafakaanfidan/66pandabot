...
client.on("guildBanAdd", async(guild, user) => {
  ...
  let limit = await db.fetch(`banlimit31_${guild.id}`)
  let ban = await db.fetch(`banlimitP31_${entry.executor.id}`)
...
client.on("roleDelete", async(role) => {
  ...
  let ullanıcılimit = await db.fetch(`rlimitP31_${entry.executor.id}`)
  const log = db.fetch(`korumaLog_${guild.id}`); 
    if(kanallimit) {
      if(entry.executor.id !== guild.owner.user.id) {
...
client.on('guildMemberAdd',async member => {
  ...
  const resim1 = await Canvas.loadImage('https://cdn.discordapp.com/attachments/724244421989892097/724270444404015134/guvenli.png')
    const resim2 = await Canvas.loadImage('https://cdn.discordapp.com/attachments/724244421989892097/724270452238843974/supheli.png')
           ...
           if (message.member.roles.has(yetkirol)) {       
               ...
               if (message.member.roles.has(yetkirol)) {       
                 ...
                 const embed2 = new Discord.RichEmbed()
                  ...
                  .addField(` Silindi`, message.guild.roles.get(vrol).name)
...
client.on("message", msg => {
  ...
  var regex = new RegExp("^(http[s]?:\\/\\/(www\\.)?|ftp:\\/\\/(www\\.)?|www\\.){1}([0-9A-Za-z-\\.@:%_\+~#=]+)+((\\.[a-zA-Z]{2,3})+)(/(.)*)?(\\?(.)*)?");
   ...
   msg.delete()
    ...
    .setDescription(`${msg.author} Reklam Yasak Bunu Bilmiyormusun! ${client.emojis.get("724240001256128572")}`)
...
client.on("message", msg => {
  ...
  if (!reklama) return
    const kelime = ["discord.gg", "discord.me", "discordapp.com", "discord.io", "discord.tk", "youtube.com"];
...
let i = db.fetch(`goldsure_${msg.author.id}`)
  ...
  .setAuthor(`mc-AT`,`${msg.author.avatarURL || msg.author.displayAvatarURL}`)
...
client.on('guildMemberAdd',async member => {
  let kisi = client.users.get(member.id);
    ...
    if (zaman < 2592000001) 
  member.addRole('724265448019066910')
  member.removeRole('724235902519345344')
});
...
let channel = client.channels.get("724266600185856118")
