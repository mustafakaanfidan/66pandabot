...
if (!message.member.hasPermission("MANAGE_CHANNELS")) return message.channel.send(`Bu komutu kullanabilmek için "\`Kanalları Yönet\`" yetkisine sahip olmalısın.`);
  message.guild.createChannel("Geçici Sohbet Odası", "category").then(kategori => {
    message.guild.createChannel("Kendi Odanı Kur", "voice").then(ses => {
  ...
  message.channel.send("Başarıyla geçici oda sistemi aktif edildi! " + client.emojis.get("724231139874701372"))
};
